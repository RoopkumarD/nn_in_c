        ...
        for (int b = 0; b < batch_num; b++) {
            ...
            for (int j = 0; j < compute_layers; j++) {
                ...
                if (failure != 0) {
                    LINE_FILE_PRINT(2);
                    fprintf(stderr,
                            "Wasn't able to matrix_mul wb[%d] and "
                            "activations[%d]\n",
                            j, j);
                ...
                if (failure != 0) {
                    LINE_FILE_PRINT(2);
                    fprintf(stderr,
                            "Wasn't able to get sigmoid of zs and "
                            "activations[%d]\n",
                            j + 1);
                    fprintf(stderr, "Training at layer %d, epoch %d\n", j + 1,
                            i);
                ...
                if (j != compute_layers - 1) {
                    ...
                    int start_posn = (activations[j + 1]->rows - 1) *
                                     activations[j + 1]->cols;
            ...
            double cost_value = sqr_cost_function(activations[num_layers - 1],
                                                  y_cost, batch_length);
            ...
            double weighted_learn_rate =
                -1 * ((double)LEARN_RATE / batch_length);
            ...
            if (failure != 0) {
                LINE_FILE_PRINT(2);
                fprintf(stderr,
                        "Wasn't able to mat_mul delta and activations[%d]\n",
                        num_layers - 2);
            ...
            for (int m = 2; m < num_layers; m++) {
                ...
                int failure =
                    matrix_mul(wb[compute_layers - m + 1], delta, delta_mul);
                if (failure != 0) {
                    LINE_FILE_PRINT(2);
                ...
                if (failure != 0) {
                    ...
                    fprintf(
                        stderr,
                        "Wasn't able to matrix_multiply delta_mul and zs\n");
                    fprintf(stderr, "Training -> delta of layer %d\n",
                            compute_layers - m);
                    retval = 2;
                    goto cleanup;
                ...
                failure = matrix_mul(delta_mul, activations[num_layers - m - 1],
                                     delta_wb);
                if (failure != 0) {
                    LINE_FILE_PRINT(2);
                    fprintf(
...
int gradient_descent(Matrix *X_train, Matrix *Y_train, int num_layers,
    ...
    for (int i = 0; i < compute_layers - 1; i++) {
        Matrix *acti =
            Matrix_create(layers[i + 1] + EXTRA_ONE, training_length);
        if (acti == NULL) {
            puts("Failed to create activation matrix");
            retval = 1;
            goto cleanup;
