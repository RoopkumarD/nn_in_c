...
int main(void) {
    ...
    puts("Reading CSV");
    /*
    failure = read_csv("./csvs/mnist_train.csv", "0", &X_mat, &Y_mat,
    EXTRA_ONE); if (failure != 0) { LINE_FILE_PRINT(2); fprintf(stderr, "Failed
    to read CSV!!!\n"); retval = failure; goto cleanup;
    }
    ...
    */
    ...
    /*
    failure = read_csv("./csvs/mnist_test.csv", "0", &X_test, &Y_test,
    EXTRA_ONE); if (failure != 0) { LINE_FILE_PRINT(); fprint(stderr, "Failed to
    read test CSV!!!\n"); retval = failure; goto cleanup;
    }
    */
    ...
    for (int i = 0; i < compute_layers; i++) {
        ...
        Matrix *weights_bias =
            Matrix_create(layers[i + 1], layers[i] + EXTRA_ONE);
    ...
    failure = stochastic_gd(X_mat, Y_mat, num_layers, wb, layers,
...
void feed_forward(Matrix *train_x, Matrix *Y_mat, int training_length,
    ...
    if (activations == NULL) {
        LINE_FILE_PRINT(2);
        fprintf(stderr,
                "Failed to create activations matrix for feedforward\n");
        goto cleanup;
    }
