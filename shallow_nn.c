...
void feed_forward(Matrix *train_x, int training_length, Matrix *weights[],
                  Matrix *bias[], int num_layer);
...
int main(void) {
    ...
    */
    ...
    Matrix *delta_weights =
        Matrix_create(high_value_of_nodes, high_value_of_nodes);
    ...
    for (int i = 0; i < EPOCH; i++) {
        ...
        print(
            f"Epoch {ep} -> Cost {np.sum((activations[-1] - training_y) ** 2) *
        0.5/len(training_x)}"
        )
        */
        double cost_value =
            cost_function(activations[num_layers - 1], Y_mat, training_length);
        printf("Epoch %d -> Cost %f\n", i, cost_value);

        // delta = (activations[-1] - training_y) * sigmoid_prime(zs[-1])
        delta->rows = activations[num_layers - 1]->rows;
        int failure =
            matrix_add(activations[num_layers - 1], Y_mat, delta, 1, -1);
        if (failure) {
        ...
        double weighted_learn_rate =
            -1 * ((double)LEARN_RATE / training_length);
        ...
        for (int k = 0; k < total_elem; k++) {
            bias[compute_layers - 1]->data[k / delta->cols] +=
                (delta->data[k] * weighted_learn_rate);
        }
        ...
        int able_to_multiply =
            matrix_mul(delta, activations[num_layers - 2], delta_weights);
        if (able_to_multiply == 1) {
            puts("mat1:");
            matrix_print(delta);
            puts("mat2:");
        ...
        for (int k = 0; k < total_elem; k++) {
            weights[compute_layers - 1]->data[k] +=
                (delta_weights->data[k] * weighted_learn_rate);
        }
        ...
        for (int m = 2; m < num_layers; m++) {
            ...
            */
            ...
            int able_to_multiply =
                matrix_mul(weights[compute_layers - m + 1], delta, delta_mul);
            if (able_to_multiply == 1) {
                puts("mat1:");
            ...
            for (int k = 0; k < total_elem; k++) {
                bias[compute_layers - m]->data[k / delta_mul->cols] +=
                    (delta_mul->data[k] * weighted_learn_rate);
            }
            ...
            matrix_mul(delta_mul, activations[num_layers - m - 1],
                       delta_weights);
            matrix_transpose(activations[num_layers - m - 1]);
            // w - ((self.learn_rate / len(training_x)) * dw)
            // matrix_scalar_mul(delta_weights, weighted_learn_rate);
            total_elem = delta_weights->cols * delta_weights->rows;
            for (int k = 0; k < total_elem; k++) {
                weights[compute_layers - m]->data[k] +=
                    (delta_weights->data[k] * weighted_learn_rate);
            }
